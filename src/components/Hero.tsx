...
export const Hero: React.FC<HeroProps> = ({ onExploreClick }) => {
  ...
  const containerRef = useScrollReveal<HTMLDivElement>(0.05);
  ...
  // Extremely subtle, GPU-friendly parallax offset for ambient background blobs
  ...
  return (
    <section className="relative min-h-[80vh] sm:min-h-[85vh] md:min-h-[90vh] flex items-center justify-center pt-24 sm:pt-28 pb-12 sm:pb-16 px-4 sm:px-6 lg:px-8 overflow-hidden max-w-full">
      ...
      {/* Hero Content Box with Staggered Scroll Reveal */}
      <div 
        ref={containerRef}
        className="relative z-10 max-w-4xl mx-auto text-center space-y-6 sm:space-y-8 scroll-reveal"
      >
        
        {/* 1. Decorative Badge */}
        <div className="inline-flex items-center gap-1.5 sm:gap-2 px-3.5 sm:px-4 py-1.5 rounded-full bg-white/90 border border-pink-200/70 shadow-pink-subtle backdrop-blur-md animate-gentleFloat">
        ...
        <div className="pt-8 sm:pt-12 hidden sm:block">
          <button
            ...
            className="p-3 rounded-full border border-pink-200/50 text-ahqar-muted hover:text-ahqar-accent hover:border-pink-300 transition-all duration-300 animate-gentleFloat active-press"
          >
