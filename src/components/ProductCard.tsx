...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onSelect }) => {
  ...
  return (
    <div className="group relative bg-white rounded-2xl overflow-hidden border border-pink-100/70 shadow-pink-subtle hover:shadow-pink-hover transition-all duration-500 flex flex-col h-full transform hover:-translate-y-1">
      
      {/* Soft Pink Hover Border Glow */}
      <div className="absolute inset-0 rounded-2xl border-2 border-transparent group-hover:border-pink-200/80 transition-colors duration-500 pointer-events-none z-20" />
      ...
      <div className="relative h-64 sm:h-72 lg:h-80 w-full overflow-hidden bg-gradient-to-b from-pink-50/90 via-rose-50/30 to-white flex items-center justify-center p-3">
        ...
        {product.badge && (
          <div className="absolute top-3 left-3 z-10">
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] uppercase tracking-widest font-semibold bg-white/90 text-ahqar-deep border border-pink-200/80 shadow-sm backdrop-blur-md">
